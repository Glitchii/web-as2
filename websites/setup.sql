...
CREATE OR REPLACE TRIGGER trig_delete_account_jobs
AFTER DELETE ON account
FOR EACH ROW
BEGIN
    DELETE FROM job WHERE accountId = OLD.id;
END;

-- Delete all jobs associated with a category when the category is deleted
CREATE OR REPLACE TRIGGER trig_delete_category_jobs
AFTER DELETE ON category
FOR EACH ROW
BEGIN
    DELETE FROM job WHERE categoryId = OLD.id;
END;

-- Delete all applicants associated with a job when the job is deleted. (CVs are stored in the file system. They will be deleted through the PHP endpoint.)
CREATE OR REPLACE TRIGGER trig_delete_job_applicants
AFTER DELETE ON job
FOR EACH ROW
BEGIN
    DELETE FROM applicant WHERE jobId = OLD.id;
END;
...
INSERT INTO account (isAdmin, username, password)
            -- Default staff/admin account. Password is 'letmein'
    VALUES  (true, 'admin', '$2y$10$arD3MUTYfY4CwuS8X7pDmOmNWAA2F3342OQmqAU8ejD.2QfM12CLS'),
            -- Test client account. Password is 'pwd123'
            (false, 'testclient', '$2y$10$CwJ7HscpYhLQQNgGMARs1u4q4Dii/jnLJ84vBX57Ms7TMlNIUOpk.'),
            -- Test staff/admin account. Password is 'pwd321'
            (true, 'teststaff', '$2y$10$0TeUB99Lbzs/zbdzrCQmNOVbnnomLT2DXOE2O9kubBWPviigwDLTW');
